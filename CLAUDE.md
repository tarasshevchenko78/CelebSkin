...
### Для агента С доступом к серверу (Claude Code на AbeloHost)
```bash
# 1. Frontend (Next.js)
cd /opt/celebskin/site && npm run build && pm2 restart celebskin

# 2. Pipeline скрипты → Contabo
rsync -avz --delete scripts/ root@161.97.142.117:/opt/celebskin/scripts/ --exclude=tmp --exclude=node_modules --exclude=.env

# 3. Git
cd /opt/celebskin/site && git add -A && git commit -m "описание" && git push origin master
```

### Для агента БЕЗ доступа к серверу
1. Клонируй: `git clone git@github.com:tarasshevchenko78/CelebSkin.git`
2. Вноси изменения в код
3. Пуш: `git push origin master`
4. **НЕТ auto-deploy** — после пуша нужен агент с доступом к серверу чтобы:
   - `git pull` на AbeloHost
   - `npm run build && pm2 restart celebskin`
   - `rsync` скриптов на Contabo
5. Сайт: `https://celeb.skin`
6. Админка: `https://celeb.skin/admin` (Basic Auth: admin / r80ENNRzb5uJDP3XWkI52j3G)

### Связь между серверами
- AbeloHost → Contabo: `ssh root@161.97.142.117` (SSH ключ настроен)
- API routes дёргают pipeline на Contabo через SSH
- БД (PostgreSQL) на AbeloHost, pipeline скрипты на Contabo подключаются к ней через `DATABASE_URL`
