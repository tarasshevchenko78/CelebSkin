...
- **Строгий порядок конвейера** — publish принимает ТОЛЬКО видео прошедшие полный pipeline (watermark → thumbnails → CDN)
- **Per-video activeItems прогресс** — каждый шаг показывает индивидуальный прогресс для каждого видео
- **CDN URL исправлен** — все URL'ы используют `celebskin-cdn.b-cdn.net` (кастомный `cdn.celeb.skin` имеет сломанный SSL)
