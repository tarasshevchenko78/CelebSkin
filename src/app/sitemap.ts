  ...
  try {
    const [videosResult, celebritiesResult, moviesResult, blogResult, collectionsResult, tagsResult] = await Promise.all([
      ...
      pool.query<SimpleSlugRow>(
        `SELECT slug, updated_at FROM celebrities WHERE status = 'published' AND videos_count > 0 LIMIT 5000`
      ),
      pool.query<SimpleSlugRow>(
        `SELECT slug, updated_at FROM movies WHERE status = 'published' AND scenes_count > 0 LIMIT 5000`
      ),
      pool.query<SimpleSlugRow>(
